      ...
      <main className="container mx-auto px-6 py-8">
        ...
        {latestRun && (
          <div className="glass-panel p-6 rounded-xl mb-8">
            ...
            <div className="grid md:grid-cols-5 gap-4">
              <div className="p-4 rounded-lg" style={{ background: 'rgba(98, 23, 8, 0.3)' }}>
                <div className="flex items-center justify-between mb-2">
                  <Bug className="w-5 h-5" style={{ color: '#941B0C' }} />
                  <span className="text-2xl font-mono font-bold" style={{ color: '#E6E1D7' }}>{latestRun.issues_found}</span>
                </div>
                <p className="text-xs" style={{ color: '#B8B3A8' }}>Issues Found</p>
              </div>

              <div className="p-4 rounded-lg" style={{ background: 'rgba(98, 23, 8, 0.3)' }}>
                <div className="flex items-center justify-between mb-2">
                  <Wrench className="w-5 h-5" style={{ color: '#F6AA1C' }} />
                  <span className="text-2xl font-mono font-bold" style={{ color: '#E6E1D7' }}>{latestRun.fixes_attempted}</span>
                </div>
                <p className="text-xs" style={{ color: '#B8B3A8' }}>Fixes Attempted</p>
              </div>

              <div className="p-4 rounded-lg" style={{ background: 'rgba(98, 23, 8, 0.3)' }}>
                <div className="flex items-center justify-between mb-2">
                  <CheckCircle2 className="w-5 h-5 text-green-400" />
                  <span className="text-2xl font-mono font-bold" style={{ color: '#E6E1D7' }}>{latestRun.fixes_succeeded}</span>
                </div>
                <p className="text-xs" style={{ color: '#B8B3A8' }}>Fixes Succeeded</p>
              </div>

              <div className="p-4 rounded-lg" style={{ background: 'rgba(98, 23, 8, 0.3)' }}>
                <div className="flex items-center justify-between mb-2">
                  <CheckCircle2 className="w-5 h-5" style={{ color: '#BC3908' }} />
                  <span className="text-2xl font-mono font-bold" style={{ color: '#E6E1D7' }}>{latestRun.verifications_passed}</span>
                </div>
                <p className="text-xs" style={{ color: '#B8B3A8' }}>Tests Passed</p>
              </div>

              <div className="p-4 rounded-lg" style={{ background: 'rgba(98, 23, 8, 0.3)' }}>
                <div className="flex items-center justify-between mb-2">
                  <Clock className="w-5 h-5" style={{ color: '#8A8780' }} />
                  <span className="text-2xl font-mono font-bold" style={{ color: '#E6E1D7' }}>
                    {latestRun.total_duration_seconds.toFixed(1)}s
                  </span>
                </div>
                <p className="text-xs" style={{ color: '#B8B3A8' }}>Duration</p>
              </div>
            </div>
          </div>
        )}
        ...
        {issues.length > 0 && (
          <div className="glass-panel p-6 rounded-xl mb-8">
            ...
            <div className="space-y-3">
              {issues.map((issue) => (
                <div key={issue.id} className="p-4 rounded-lg border hover:border-opacity-50 transition-colors" style={{ background: 'rgba(98, 23, 8, 0.2)', borderColor: 'rgba(148, 27, 12, 0.3)' }}>
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <span className={`px-2 py-1 rounded text-xs font-mono border ${getSeverityColor(issue.severity)}`}>
                        {issue.severity.toUpperCase()}
                      </span>
                      <code className="text-sm" style={{ color: '#B8B3A8' }}>{issue.file}</code>
                  ...
                  <p className="text-sm mb-2" style={{ color: '#B8B3A8' }}>{issue.description}</p>
                  <div className="flex items-center gap-2">
                    {issue.detectors.map((detector) => (
                      <span key={detector} className="px-2 py-0.5 rounded text-xs border" style={{ background: 'rgba(246, 170, 28, 0.1)', color: '#F6AA1C', borderColor: 'rgba(246, 170, 28, 0.3)' }}>
                        {detector}
                      </span>
                    ))}
                  </div>
        ...
        {fixes.length > 0 && (
          <div className="glass-panel p-6 rounded-xl mb-8">
            ...
            <div className="space-y-3">
              {fixes.map((fix) => (
                <div key={fix.fix_id} className="p-4 rounded-lg border" style={{ background: 'rgba(98, 23, 8, 0.2)', borderColor: 'rgba(148, 27, 12, 0.3)' }}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-mono" style={{ color: '#B8B3A8' }}>{fix.issue_id}</p>
                        <p className="text-xs" style={{ color: '#8A8780' }}>
                          {fix.codex_live ? (
                            <span style={{ color: '#F6AA1C' }}>● Live Codex</span>
                          ) : (
                            <span style={{ color: '#8A8780' }}>○ Cached</span>
                          )}
                        </p>
                      </div>
                    </div>
                    <span className="text-xs" style={{ color: '#8A8780' }}>{fix.duration_seconds.toFixed(2)}s</span>
                  </div>
                  <p className="text-sm mb-2" style={{ color: '#B8B3A8' }}>{fix.summary}</p>
                  {fix.changed_files.length > 0 && (
                    <div className="flex items-center gap-2 flex-wrap">
                      {fix.changed_files.map((file, idx) => (
                        <code key={idx} className="text-xs px-2 py-1 rounded" style={{ background: '#220901', color: '#F6AA1C' }}>
                          {file}
                        </code>
                      ))}
                    </div>
          ...
          <div className="glass-panel p-6 rounded-xl">
            ...
            <div className="space-y-3">
              {verdicts.map((verdict) => (
                <div key={verdict.verdict_id} className={`p-4 rounded-lg border ${verdict.tests_passed ? 'border-green-500/20' : 'border-red-500/20'}`} style={{ background: verdict.tests_passed ? 'rgba(16, 185, 129, 0.05)' : 'rgba(220, 38, 38, 0.05)' }}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-mono" style={{ color: '#B8B3A8' }}>{verdict.issue_id}</span>
                    </div>
                    <span className="text-xs" style={{ color: '#8A8780' }}>
                      {(verdict.confidence * 100).toFixed(0)}% confidence
                    </span>
                  </div>
                  <p className="text-sm" style={{ color: '#B8B3A8' }}>{verdict.explanation}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {!latestRun && !isRunning && (
          <div className="glass-panel p-12 rounded-xl text-center">
            <Terminal className="w-16 h-16 mx-auto mb-4 opacity-50" style={{ color: '#F6AA1C' }} />
